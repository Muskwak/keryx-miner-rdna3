...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // RDNA3 fork: no nvcc / CUDA PTX build. The PoM walk + kHeavyHash PoW run as Vulkan compute
    // shaders compiled by the keryx-vulkan crate (glslc → SPIR-V); nothing CUDA is built here.
